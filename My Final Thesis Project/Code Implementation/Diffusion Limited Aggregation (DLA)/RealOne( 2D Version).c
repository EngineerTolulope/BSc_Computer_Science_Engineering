    ...
    for (i=0;i<N;i++){
      ...
      if (i<L){                     //Storing the bottom pixels as the landmark
        ...
        if (i%25==0){
            ...
            //
            sprintf(name,"Pictures Folder\\Sim%d.pov",i);//Creating pov files after every 25th simulation as well
            fp=fopen(name, "w");
            ...
            for (j=0;j<=i;j++) fprintf(fp, "sphere { <%d,%d,0>,0.500\ntexture{ pigment{ color rgb<%f,0.0,0.5>}\nfinish{ phong 1 metallic}}}\n",part[j].x,part[j].y,j/(float)N);

            fprintf(fp,"rotate y*0\ntranslate <0,0,0>\n}\n");
            fprintf(fp,"camera{\nlocation <%d,%d,%d>\nlook_at  <%d,%d,0>}\n" ,L/2,hmax/2,L-50 ,L/2,hmax/2);
            fclose(fp);
        }//If
