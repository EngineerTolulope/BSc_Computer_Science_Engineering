...
//Including Padding
   ...
   unsigned char *p=(unsigned char*)malloc(RowSize);//Array to store the rgb colours
   while (n < H){ //repeat the loop until you reach the height
      read(f,p,RowSize); //read from the file and store it in p including the padding, incase if there is padding
      for (i=0; i<3*W; i+=3){ //Read as much only the row excluding padding
                              //Only the part without padding are modified
         temp   = p[i] + p[i+1] + p[i+2];
         temp  /= 3;
         p[i]   = temp;
         p[i+1] = temp;
      ...
      write(g,p,RowSize); //write the new p with the padding included as well
