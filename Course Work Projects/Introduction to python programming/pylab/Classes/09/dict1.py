...
def main():
    ...
    #pass
    d={} #empty dictionary
    for word in (TEXT.lower()).split():#converts all letters to lowercase, then spilts the TEXT to give a list
        if word in d:
            d[word]+=1 #increments the value of the word
                        #value of the word is a number
        else:
            d[word]=1
